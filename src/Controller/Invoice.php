...
class Invoice
{
    ...
    public static function sendTodayAlerts(): Http\JsonResponse
    {
        ...
        try {
            ...
            $alerts = $em->createQueryBuilder()
                ->select('a')
                ->from(\App\Entity\InvoiceFeedAlert::class, 'a')
            ...
            /** @var InvoiceFeedAlert $alert */
            foreach ($alerts as $alert) {
                $body .= "<h4>" . $alert->feed->invoice->enterprise . "</h4>";
    ...
    {
        try {
            $list = \App\Entity\Invoice::findBy(['deletedOn' => null]);
            $origen = [];
    ...
    public static function filterAllFeed(int $id): Http\JsonResponse
    {
        try {
            ...
            $qb = $em->getRepository(\App\Entity\InvoiceFeed::class)
                ->createQueryBuilder('i')
                ->where('i.invoice = :id')
     ...
     */
    public static function notifyCreated(\App\Entity\Invoice $item): void
    {
        ...
        foreach ($employees as $employee) {
            ...
            Mail::send($employee->email, 'Nueva factura en CRM', ['html' => $body, 'text' => $text]);
        }
     ...
     * @throws \PHPMailer\PHPMailer\Exception
     */
    public static function notifyEdited(\App\Entity\Invoice $item): void
    {
        // Buscar receptores de la notificación
        ...
        foreach ($employees as $employee) {
            ...
            $mail->addEmbeddedImage(FileSystem::getPath(Directory::PUBLIC) . '/img/logo/logo-blue.png', 'logo');
            Mail::send($employee->email, 'Factura actualizada en CRM', ['html' => $body, 'text' => $text]);
        }
    }
     ...
     * @throws ModuleException
     */
    public static function notifyDeleted(\App\Entity\Invoice $item): void
    {
        // Buscar receptores de la notificación
        ...
        foreach ($employees as $employee) {
            ...
            $mail = Mail::getInstance();
            $mail->addEmbeddedImage(FileSystem::getPath(Directory::PUBLIC) . '/img/logo/logo-blue.png', 'logo');
            Mail::send($employee->email, 'Factura eliminada en CRM', ['html' => $body, 'text' => $text]);
        }
    ...
    public static function get(int $id): Http\JsonResponse
    {
        try {
            ...
            $item = \App\Entity\Invoice::findOneBy(['id' => $id, 'deletedOn' => null]);
    ...
    public static function getFeed(int $id): Http\JsonResponse
    {
        try {
            ...
            $item = \App\Entity\InvoiceFeed::findOneBy(['id' => $id, 'deletedOn' => null]);
    ...
    public static function getAlert(int $id): Http\JsonResponse
    {
        try {
            ...
            $item = \App\Entity\InvoiceFeedAlert::findOneBy(['feed' => $id, 'deletedOn' => null]);
    ...
    public static function updateAlert(int $id): Http\JsonResponse
    {
        try {
            ...
            $item = \App\Entity\InvoiceFeedAlert::findOneBy(['id' => $id, 'deletedOn' => null]);
     ...
     * @throws ModuleException
     */
    public static function getNotificationReceivers(string $roleReceive, \App\Entity\Invoice $item): array
    {
        $employees = [];
